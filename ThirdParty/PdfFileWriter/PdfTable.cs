...
public class PdfTable
	...
	public void DrawRow()
		...
		// test for new page
		if(RowEnd < _TableArea.Bottom) CreateNewPage();
	...
	// Draw borders and grid lines after the last row on a page is drawn
	// or the last row of the table is drawn.
	private void DrawBorderAndGrid()
		{
		// roll back row position to end of previous row
		_RowTopPosition += 2.0 * HalfGridLineExtra;

		// no border or grid lines
		if(_BorderLineControl == BorderControl.None && _GridLineControl == BorderControl.None) return;

		// adjust row position to center of border line
		_RowTopPosition -= HalfBorderLineExtra;

		// border top and bottom
		Double BorderTop = GridYPos[0];
		Double BorderBottom = _RowTopPosition;

		// save graphics state
		Contents.SaveGraphicsState();

		// we have border lines
		if(_BorderLineControl != BorderControl.None)
			{
			// set line width and colour
			Contents.SetLineWidth(_BorderLineWidth);
			Contents.SetColorStroking(BorderLineColor);

			// draw border rectangle
			if(_BorderLineControl == BorderControl.Both)
				{
				Contents.DrawRectangle(_TableArea.Left, BorderBottom, _TableArea.Width, BorderTop - BorderBottom, PaintOp.CloseStroke);
				if(DisplayHeader) Contents.DrawLine(_TableArea.Left, GridYPos[1], _TableArea.Right, GridYPos[1]);
				}

			// draw horizontal lines
			else if(_BorderLineControl == BorderControl.Horizontal)
				{
				Contents.DrawLine(_TableArea.Left, BorderBottom, _TableArea.Right, BorderBottom);
				Contents.DrawLine(_TableArea.Left, BorderTop, _TableArea.Right, BorderTop);
				if(DisplayHeader) Contents.DrawLine(_TableArea.Left, GridYPos[1], _TableArea.Right, GridYPos[1]);
				}
			...
			// draw vertical lines
				...
				{
				Contents.DrawLine(_TableArea.Left, BorderBottom, _TableArea.Left, BorderTop);
				Contents.DrawLine(_TableArea.Right, BorderBottom, _TableArea.Right, BorderTop);
				}
			}

		// we have grid lines
		if(_GridLineControl != BorderControl.None)
			{
			// set line width and colour
			Contents.SetLineWidth(_GridLineWidth);
			Contents.SetColorStroking(GridLineColor);

			// draw vertical lines
			if((_GridLineControl & BorderControl.Vertical) != 0)
				{
				for(Int32 Col = 1; Col < Columns; Col++)
					{
					Contents.DrawLine(_ColumnPosition[Col], BorderBottom, _ColumnPosition[Col], BorderTop);
					}
				}

			// draw horizontal lines
			if((_GridLineControl & BorderControl.Horizontal) != 0)
				{
				Int32 RowStart = (DisplayHeader && (_BorderLineControl & BorderControl.Horizontal) != 0) ? 2 : 1;
				Int32 RowEnd = GridYPos.Count - 1;
				for(Int32 Row = RowStart; Row < RowEnd; Row++)
					{
					Contents.DrawLine(_TableArea.Left, GridYPos[Row], _TableArea.Right, GridYPos[Row]);
					}
				}
			}
